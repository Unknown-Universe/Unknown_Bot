...
const welcome: Command = {
    ...
    run: async (message, ...args) => {
        ...
        switch (modifier) {
            case "send":
                if (!args.length || !(args[0].toLowerCase() === "true" || args[0].toLowerCase() === "false")) {
                ...
                await db.execute('UPDATE `guilds` SET `send_welcome` = ? WHERE `id` = ?', [message.guildId, args[0].toLowerCase() === "false" ? "0":"1"]);
                await message.reply(`I will ${args[0].toLowerCase() === "false" ? "no longer welcome":"now welcome"} new members`);
                return;
            case "channel":
                if (!args.length) {
                ...
                await db.execute('UPDATE `guilds` SET `welcome_channel` = ? WHERE `id` = ?', [channelID, message.guildId]);
            ...
            case "message":
                ...
                await db.execute('UPDATE `guilds` SET `welcome_message` = ? WHERE `id` = ?', [args.join(' '), message.guildId]);
